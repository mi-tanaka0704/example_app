...
- constの置き場所は基本あまり関係ない。
- constは「実行せずに確定できるか= コンパイル時定数」で決まる（実行時のタイミングには依存しない）。
- そのため、build()が何回呼ばれても、const Text('Hello') は同じ定数として扱われる。
  - 今回の場合は、TextのWidgetオブジェクトが定数として扱われる
  - build() 自体は何度でも呼ばれるが、そこで返すconst Text('Hello') は毎回同じ定数扱い
```Dart
@override
